...
void sort_post_distribution(struct tape *tape);
...
int opposite_tape_index(int index) {
    return 1 - index;
}
...
int sort_distribution_phase2(const char *input_file_path, int print_contents) {
    ...
    // while there are still elements on the input tape
    while (1) {
        // extra records
        if (record_is_empty(&tapes[active_tape].extra_record)) {
            disk_get_next_record(&input, &record);
        } else {
            record = tapes[active_tape].extra_record;
            generate_incorrect_record(&tapes[active_tape].extra_record);
        }

        // merge runs to one tape (based on fib_prev)
        while (tapes[active_tape].num_runs != fib_prev) {
            // struct record prevprev = tapes[active_tape].last_record;
            ...
            // record_print(&tapes[active_tape].last_record, RECORD_PRINT_ID);
            // record_print(&record, RECORD_PRINT_ID | RECORD_PRINT_EMPTY_RECORDS);
            // printf("\n");
        ...
        tapes[opposite_tape_index(active_tape)].extra_record = record;
        //tapes[active_tape].num_runs += 1;
    ...
    // printf("active tape %d\n", active_tape);
    sort_post_distribution(&tapes[active_tape]);
    sort_post_distribution(&tapes[opposite_tape_index(active_tape)]);

    if (tapes[active_tape].num_runs != fib_next - fib_prev)
        tapes[active_tape].dummy_runs = fib_prev - tapes[active_tape].num_runs;
    if (tapes[active_tape].num_runs == fib_next - fib_prev)
...
void sort_postprocess_phase(int initial_elements_num, int print_contents, int used_keyboard) {
    ...
    printf("    read  %d\n", get_read_operations_number());
    printf("    write %d\n", get_write_operations_number());
    printf("    sum   %d\n", get_read_operations_number() +  get_write_operations_number());
    ...
    if (used_keyboard || initial_elements_num < 50000)
        initial_elements_num_corrected = initial_elements_num;
...
void sort_post_distribution(struct tape *tape) {
    ...
    generate_incorrect_record(&prev);
    int runs = 1;
    int tmp1 = get_read_operations_number();
    int tmp2 = get_write_operations_number();
    ...
    set_read_operations_number(tmp1);
    set_write_operations_number(tmp2);
    tape->num_runs = runs;
}
